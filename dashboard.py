...
# --- NEW LIVE DATA LOADING SECTION ---

# PASTE YOUR "PUBLISH TO WEB" CSV LINK (from Step 1) HERE:
...
def load_data(url):
    """Loads data from the Google Sheet "Publish to web" CSV link."""
    try:
        # Use ISO-8859-1 encoding for compatibility
        return pd.read_csv(url, encoding="ISO-8859-1")
    except Exception as e:
        st.error(f"Error loading data from Google Sheets URL: {e}")
        st.info("Please ensure your 'Publish to web' CSV link is correct and pasted in DATA_URL.")
        return pd.DataFrame()
...
if df.empty:
    st.error("Data could not be loaded. Please check your Google Sheet 'Publish to web' link.")
    st.stop()
...
# --- (The rest of your file is exactly the same as before) ---
# --- Sidebar filters ---
...
if not filtered_df.empty:
    try:
        ...
        segment_map = {
            ...
            r'3[1-2]': 'Needs Attention', r'33': 'About to Sleep',
            r'[3-4]4': 'Loyal Customers', r'4[1-2]': 'Promising',
            r'43': 'Potential Loyalists', r'44': 'Champions'
        }
        ...
        fig_segment = px.bar(
            segment_counts, x='Segment', y='Customer Count',
            title="Number of Customers by Segment", text='Customer Count', template="seaborn"
        )
        ...
        with st.expander("View RFM Data and Segment Definitions"):
            ...
            - **Loyal Customers ([3-4]4):** Buy on a regular basis.
            - **Potential Loyalists (43):** Recent customers, good spend.
            - **Promising (4[1-2]):** Recent shoppers, but haven't spent much.
            - **Needs Attention (3[1-2]):** May not have bought very recently.
            - **About to Sleep (33):** Below average. Will be lost if not reactivated.
            - **At-Risk ([1-2][3-4]):** Purchased often and spent big, but it’s been a while.
            - **Hibernating ([1-2][1-2]):** Last purchase was long back, low spenders.
            """)
            st.dataframe(rfm_df)
    except Exception as e:
        st.error(f"An error occurred during RFM Analysis. This can happen with small datasets. Error: {e}")
else:
        ...
        with st.spinner("Training model and generating forecast..."):
            try:
                ...
                with st.expander("View Forecast Data"):
                    st.write("Predicted sales (`yhat`) with confidence bounds.")
